...
const (
	...
	ES_URL      = "http://34.67.230.131:9200"
	BUCKET_NAME = "oliver-bucket-12345"
)
...
type Location struct {
	//this is jason format syntax. Lat appers in JSON as key "lat"
...
func handlerPost(w http.ResponseWriter, r *http.Request) { // this method automatically produce a object p from jason formation r.body
...
func getPostFromSearchResult(searchResult *elastic.SearchResult) []Post {
	...
	for _, item := range searchResult.Each(reflect.TypeOf(ptype)) { //for i, v := range []   i is index, starting from 0; v is the coresponding value
		p := item.(Post)
